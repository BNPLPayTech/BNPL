...
import "@openzeppelin/contracts-upgradeable/token/ERC20/ERC20Upgradeable.sol";// the imported libraries are affected by the upgradable contract so it needs to change to 
...
// Check line 140 I commented the contructor and replace it with the initalizer function
...
contract BankingNode is Initializable, ERC20Upgradeable { // there is no constructor in ERC20Upgradeable ("BNPL USD", "bUSD")
   ...
   //-----> in the upgradablde contracts, we cannot have the constructor, and instead we need to replace it the initialize function
    // here the initialize base contract take care of multiple instantiation!
    ...
    function initialize(
        ...
        require(
            __ERC20_init(_baseToken).decimals() ==
                __ERC20_init(
        ...
        nonBaseToken(collateral)
    {
        ...
        address _bnpl = BNPL;
        uint256 feesAccrued = IERC20Upgradeable( //***
    ...
    function collectFees() external {
        ...
        address _bnpl = BNPL;
        address _operator = operator;
        uint256 _operatorFees = IERC20Upgradeable(_baseToken).balanceOf(address(this)) / 3; // *** this must be changed
        ...
        nonZeroInput(_amount)
    {
        ...
        uint256 tokenDecimals = __ERC20_init(_baseToken).decimals();
        if (tokenDecimals != 18) {
    ...
    function getStakedBNPL() public view returns (uint256) {
        return
            IERC20Upgradeable(BNPL).balanceOf(address(this)) -
            unbondingAmount -
            slashingBalance;
    }
    ...
    function getTotalAssetValue() public view returns (uint256) {
        return
            IERC20Upgradeable(_getLendingPool().getReserveData(baseToken).aTokenAddress)
